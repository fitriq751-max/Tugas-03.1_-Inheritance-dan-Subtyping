...
def cetak_slip_gaji(objek_pegawai):
    ...
    table.add_row(
        "Gaji Pokok",
        f"Rp{objek_pegawai.gaji_pokok:,}"
    )

    table.add_row(
        "Total Gaji",
        f"Rp{objek_pegawai.hitung_gaji():,}"
    )
...
if __name__ == "__main__":
    ...
    console.print(
        f"[bold blue]{figlet.renderText('SMART PAYROLL')}[/bold blue]"
    )

    console.print(
        Panel.fit(
            "[bold cyan]SISTEM PENGGAJIAN KARYAWAN TERPADU[/bold cyan]\n"
            "[bold green]PT Teknologi Masa Depan[/bold green]",
            border_style="yellow"
        )
    )
    ...
    while True:
        ...
        if pilihan == "1":
            ...
            no_hp = input("No HP                   : ")
            gaji_pokok = int(
                    input("Gaji Pokok              : ").replace(".", ""))

            if jabatan == "1":

                hari_kerja = int(
                    input("Jumlah Hari Kerja       : ").replace(".", ""))

                jam_lembur = int(
                    input("Jumlah Jam Lembur       : ").replace(".", ""))   
                ...
                bonus_jabatan = int(
                    input("Bonus Jabatan           : ").replace(".", ""))
                
                tunjangan_transport = int(
                    input("Tunjangan Transport     : ").replace(".", ""))

                tunjangan_kesehatan = int(
                    input("Tunjangan Kesehatan     : ").replace(".", ""))
            ...
            console.print(
                Panel.fit(
                    "[bold magenta]TOTAL PENGELUARAN GAJI[/bold magenta]",
                    border_style="red"
                )
            )
            ...
            console.print(
                f"[bold green]"
                f"Rp{perusahaan.total_pengeluaran_gaji():,}"
                f"[/bold green]"
            )
